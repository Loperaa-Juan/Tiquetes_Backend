...
class Estudiante(_database.Base):
    ...
    viajes = _orm.relationship("Viaje", back_populates="estudiante")
...
class Administrador(_database.Base):
    ...
    viajes = _orm.relationship("Viaje", back_populates="administrador")
...
class Viaje(_database.Base):
    ...
    estudiante_id = _sql.Column(UUID(as_uuid=True), _sql.ForeignKey("estudiantes.estudiante_id"), nullable=False)
    administrador_id = _sql.Column(UUID(as_uuid=True), _sql.ForeignKey("administradores.administrador_id"), nullable=False)
    fecha_creacion = _sql.Column(_sql.DateTime, default=_dt.datetime.utcnow)
    activo = _sql.Column(_sql.Boolean, default=True, nullable=False)
    actualiza = _sql.Column(_sql.DateTime, default=_dt.datetime.utcnow, onupdate=_dt.datetime.utcnow)

    estudiante = _orm.relationship("Estudiante", back_populates="viajes")
    administrador = _orm.relationship("Administrador", back_populates="viajes")