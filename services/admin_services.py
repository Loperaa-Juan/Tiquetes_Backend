...
async def get_current_user(db: _orm.Session = Depends(_databaseServices.get_db), token: str = Depends(OAuth2_scheme)):
    try:
        ...
        user = db.query(_models.Administrador).filter(_models.Administrador.identificacion == user_id).first()  
        if user is None:
...
async def create_admin(admin: _admin.AdminCreate, db: _orm.session, admin_user: _admin.Admin = Depends(get_current_user)):

    if not admin_user:
        raise HTTPException(status_code=401, detail="Unauthorized, only administrators can create new admins")
...
async def delete_admin(admin_identification: str, db: _orm.session, admin: _admin.Admin):
    ...
    if not administrador:
        raise HTTPException(status_code=404, detail=f"El administrador con idenficación {admin_identification} no se encuentra registrada")
...
async def authenticate_admin(admin_identification: str, password: str, db: _orm.session):
...
async def edit_admin(admin: _admin.AdminCreate, db: _orm.session, admin_user: _admin.Admin = Depends(get_current_user)):
    if not admin_user:
        raise HTTPException(status_code=401, detail="Unauthorized, only administrators can edit admins")
    
    administrador = await get_admin_by_identification(id=admin.identificacion, db=db)

    if not administrador:
        raise HTTPException(status_code=404, detail=f"El administrador con identificación {admin.identificacion} no se encuentra registrado")
