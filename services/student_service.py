...
async def get_user_by_identificacion(identificacion: str, db: _orm.session, admin: _admin.Admin):
...
def update_tickets(student_identification: str, tickets_number: int, db: _orm.session, admin: _admin.Admin):
    if not admin:
        raise HTTPException(status_code=401, detail="Unauthorized")
    if not student_identification:
        raise HTTPException(status_code=400, detail="Identificacion del estudiante es requerida")
    if tickets_number < 0:
        raise HTTPException(status_code=400, detail="El numero de tiquetes debe ser mayor o igual a 0")
...
def discount_ticket(student_identification: str, db: _orm.session, admin: _admin.Admin):
    ...
    if estudiante.numero_tiquetes <= 0:
        raise HTTPException(status_code=400, detail="El estudiante no tiene tiquetes disponibles para descontar")
